...
var EnviarbaseComponent = (function () {
    function EnviarbaseComponent(user) {
        this.user = user;
        this.API_URL = "https://mylead2-api.herokuapp.com";
    }
...
var AuthComponent = (function () {
    function AuthComponent(utility, user, router, _http, _service) {
        this.utility = utility;
        this.user = user;
        this.router = router;
        this._http = _http;
    ...
    AuthComponent.prototype.registerAccount = function () {
        ...
        if (this.register.email && this.validatePasswords() && this.register.nameEnterprise) {
            if (!$(".form-item.sing-up .btn").hasClass("loading")) {
                ...
                {
                    return new Promise(function (resolve, reject) {
                        _this._http.post(_this.API_URL + '/enterprise', _this.register)
                            .subscribe(function (result) {
                            if (result.json()) {
                                if (result.json().status == "success") {
                                    _this._service.success('Sucesso', result.json().message);
    ...
    AuthComponent = __decorate([
        ...
        __metadata("design:paramtypes", [__WEBPACK_IMPORTED_MODULE_5__utility_service__["a" /* UtilityService */],
            __WEBPACK_IMPORTED_MODULE_1__user_service__["a" /* UserService */],
            __WEBPACK_IMPORTED_MODULE_2__angular_router__["a" /* Router */],
            __WEBPACK_IMPORTED_MODULE_3__angular_http__["a" /* Http */],
            __WEBPACK_IMPORTED_MODULE_4_angular2_notifications__["NotificationsService"]])
